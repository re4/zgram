...
void InnerWidget::changeOpenedForum(Data::Forum *forum) {
	...
	_openedForum = forum;
	_st = forum ? &Row::ForumTopicSt() : &Row::DefaultSt();
	refreshShownList();
...
void InnerWidget::paintEvent(QPaintEvent *e) {
	...
	const auto paintRow = [&](
		...
		context.st = (forum || monoforum) ? &Row::ForumSt() : _st.get();
...
void InnerWidget::paintPeerSearchResult(
	...
	QRect fullRect(0, 0, context.width, Row::DefaultSt().height);
	p.fillRect(
		fullRect,
		(context.active
			? st::dialogsBgActive
...
void InnerWidget::selectByMouse(QPoint globalPosition) {
	...
	if (_state == WidgetState::Default) {
		...
		if (!_peerSearchResults.empty()) {
			...
			const auto rowHeight = Row::DefaultSt().height;
			auto peerSearchSelected = (mouseY >= skip)
				? ((mouseY - skip) / rowHeight)
				: -1;
	...
	if (base::in_range(_collapsedSelected, 0, _collapsedRows.size())) {
		...
		const auto origin = e->pos()
			- QPoint(
				0,
				peerSearchOffset()
					+ _peerSearchPressed * Row::DefaultSt().height);
...
bool InnerWidget::pinnedShiftAnimationCallback(crl::time now) {
	...
	if (updateMin >= 0) {
		...
		const auto maxHeight = Row::TaggedForumSt().height;
...
void InnerWidget::updateSelectedRow(Key key) {
	...
	if (_state == WidgetState::Default) {
		...
		if (key) {
			...
			update(
				0,
				peerSearchOffset() + _peerSearchSelected * rowHeight,
				width(),
				rowHeight);
